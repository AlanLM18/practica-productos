...
class ProductoBD extends ConectarBD{
    ...
    async nuevoProducto(productos){
        const sql="insert into productos values(null,'"+productos.nom_prod+"','"+productos.categoria+"','"+productos.stock+"','"+productos.precio+"');";
        try {
    ...
    async editarProducto(producto){
        const sql1 = `update productos set nom_prod ='${producto.nom_prod}', categoria ='${producto.categoria}', stock ='${producto.stock}', precio ='${producto.precio}' where id_prod ='${producto.id_prod}';`;
        try {
            await this.conectarMySql();
